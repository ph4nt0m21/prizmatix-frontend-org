...
const SideNavBar = ({ isMobileSidebarOpen, toggleMobileSidebar }) => {
  ...
  ...(['ORGANIZER', 'SCANNER'].includes(userRole)
    ? [{ id: 'scanner', path: '/scanner', icon: ScannerIcon, label: 'Scanner' }]
    : [])
];
  ...
  const bottomItems = [
    ...(['ORGANIZER', 'SUPER_ADMIN'].includes(userRole)
    ? [
        { id: 'help', icon: HelpIcon, label: 'Help & Support', action: () => setIsHelpModalOpen(true) }
      ]
    : []),
  ];
  ...
  return (
    ...
    // If the item has an 'action' function, render a button.
    if (item.action) {
      return (
        <button
          key={item.id}
    ...
    // Otherwise, render the NavLink as before.
    return (
      <NavLink
        key={item.id}
        to={item.path}
