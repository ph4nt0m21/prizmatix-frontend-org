...
const EmailCampaignsPage = () => {
  ...
  const handleAttachmentUpload = async (e) => {
    ...
    if (oversized.length > 0) {
      toast.warning(
        `file size should not exceed ${MAX_FILE_SIZE_MB} MB and were not uploaded.`
      );
      if (fileInputRef.current) fileInputRef.current.value = null;
      return;
    }
  ...
  const handleSend = async () => {
    try {
      ...
      const effectiveCampaignId =
        campaignSavedId || campaignId || getDraftCampaignId();
      if (!effectiveCampaignId)
        return toast.error("Please save the campaign before sending.");
  ...
  const handleSendTestEmails = async () => {
    ...
    try {
      ...
      if (!effectiveCampaignId)
        return toast.error(
          "Please save the campaign before sending test emails."
        );
...
<div className={styles.pageHeader}>
  <div className={styles.breadcrumbs}>
    <span className={styles.breadcrumbRoot}>Email Campaign</span>
    <span className={styles.breadcrumbDivider}>›</span>
      ...
      <div className={styles.emailFormCard}>
        ...
        <div className={styles.cardHeader}>
          <div className={styles.cardHeaderLeft}>
            <h1 className={styles.cardTitle}>Email Campaign</h1>
            <p className={styles.cardSubtitle}>
              Enter the details of your email that will be displayed to
              attendees
            </p>
          </div>
        ...
        <div className={styles.emailFormContainer}>
          ...
          <div className={styles.formGroup}>
            ...
            <Select
              ...
              onChange={(s) =>
                setSelectedEvents(s.map((o) => o.value.toString()))
              }
              className={styles.multiSelect}
              classNamePrefix="select"
            />
  ...
  {/* Show Send Test Mail + Send Campaign only when saved */}
  {isSaved && (
    <>
      <button
        className={styles.primaryButton}
