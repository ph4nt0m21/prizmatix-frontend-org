...
const EventManageSidebar = ({
  ...
  navigateToTicketEditPage, // New prop for navigating to TicketEditPage
  eventId
}) => {
  ...
  const sections = [
    ...
    },
    {
      ...
      group: 'page' // Changed group to 'page'
    }
  ];
  ...
  const getSectionClass = (section) => {
    ...
    if (section.group === 'manage' && isActive) return `${styles.section} ${styles.active}`;
    if (section.group === 'manage' && status && status.completed) return `${styles.section} ${styles.completed}`;
    if (section.group === 'manage' && status && status.visited) return `${styles.section} ${styles.visited}`;
  ...
  const handleSectionClick = (section) => {
    ...
    } else if (section.key === 'tickets') {
      navigateToTicketEditPage();
    }
  };

  // Filter sections by group
  const manageSections = sections.filter(section => section.group === 'manage');
  const editPageSections = sections.filter(section => section.group === 'page');
...
EventManageSidebar.propTypes = {
  ...
  navigateToTicketEditPage: PropTypes.func.isRequired, // New propType
